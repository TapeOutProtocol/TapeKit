// 网关域名设置：设了之后，链上网站在「<#ID>-<处理器编号>.<网关域名>」这个真实来源下打开
...
export function targetUrl(input, gateway) {
  ...
  let m, id, cpu, rest = '';
  if ((m = s.match(/^#?([0-9]+)@([0-9]+)(\/.*)?$/))) { id = m[1]; cpu = m[2]; rest = m[3] || ''; }
  else if ((m = s.match(/^([0-9]+)\.([0-9]+)(?:\.tape)?\.?(\/.*)?$/i))) { id = m[1]; cpu = m[2]; rest = m[3] || ''; }
  else return viewer;
  if (!/^(0|[1-9][0-9]*)$/.test(id) || !/^(0|[1-9][0-9]*)$/.test(cpu) || id === '0') return viewer;
  const scheme = /^(localhost|127\.0\.0\.1)(:\d+)?$/.test(gateway) ? 'http' : 'https';
  return `${scheme}://${id}-${cpu}.${gateway}${rest || '/'}`;
}
